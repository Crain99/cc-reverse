...
- **fix(3.x): isolate runtime bundles from `assets/`, auto-detect engine version, switch scene to `.scene`.** Manual smoke-test of slgq-reverse output in Cocos Creator 3.8 Dashboard surfaced four blockers that PR #12 hadn't covered: (a) `engine3x.unpackBundle` copied each bundle's compiled SystemJS megabundle (`<bundleDir>/game.js` or `index.js`, 5MB+) into `assets/<bundle>/`, where the editor tried to evaluate it and crashed with `Module ../libs/um.js not found` (these megabundles reference Cocos engine internals through relative paths that only resolve under the original mini-game runtime); (b) `recoverScripts` walked `src/assets/` (compiled vendor SDKs, "plugs/") into `assets/Scripts/plugs/`, tripping the editor's ccclass scanner on globals decorated by the SDKs; (c) `recoverScriptsLayered` wrote post-Layer-3 raw `.js` modules into `assets/scripts/<bundle>/`, but those still carry `System.register(...)` and `__unresolved_N` bare-specifier placeholders that crash SystemJS (`Error: Unable to resolve bare specifier '__unresolved_6'`); (d) `writeCocos3xProject` had a hardcoded `'3.8.0'` fallback and `pickCocosVersion` didn't read the canonical 3.x src/settings.json key (`CocosEngine`), so a project built with Creator 3.8.8 was scaffolded as 3.8.0 and the Dashboard refused to open it (`The current project requires Cocos Creator 3.8.0`); (e) cc.SceneAsset emitted `.fire` (the 2.x extension) on 3.x outputs, but the 3.x scene importer keys off `.scene`, so the import never ran and the editor 404'd on `library/<scene-uuid>.json`. Fixes: SystemJS megabundles → `_runtime/bundle-scripts/<bundle>/`; plug SDKs → `_runtime/scripts/plugs/`; layered raw `.js` → `_runtime/scripts/<bundle>/`; `pickCocosVersion` now reads `settings.CocosEngine` first; `inferImportExt`/`inferMetaExt` return `.scene` for `cc.SceneAsset`. Tests in `test/unit/3x-bundle-runtime-isolation.test.js` (8 cases) + `test/integration/scriptRecovery.test.js` updated to assert the new `_runtime/` location. `npm test`: 150 → **158 passing**, no regressions. Verified on `~/mini/slgq-reverse`: `assets/{main,internal,resources}/` no longer contain `game.js`/`index.js`; `assets/Scripts/` not created at all (was the leak path); `package.json` carries `version: '3.8.8'` auto-detected from `src/settings.json:CocosEngine`; scene file is `assets/main/scene/game.scene` (was `game.fire`).

- **feat(3.x): make recovered project loadable in Cocos Creator 3.8 Dashboard.** `slgq-reverse` was previously analysis-only — Dashboard rejected the recovered tree because (a) `db://assets/...` and `db://internal/...` were materialised as literal `db:/` directories under `assets/`, (b) `package.json` was a 5-line skeleton missing `type`/`uuid`/`version` (Dashboard reads project identity from `package.json`, not `project.json`, in 3.x), (c) `settings/v2/packages/project.json` (the real 3.8 settings location) was never written, (d) runtime adapters (`bundle.js`, `import-map.js`, `spine.{asm,js,wasm}-*.js`, `engine-adapter*.js`) lived under `assets/Scripts/` and tripped the editor's ccclass scanner, (e) `cc.SceneAsset` entries went through the catch-all `raw/<uuid>.json` path because `resolveOutputPath` only consulted `cfg.paths` and ignored the synthetic `info.path` passed by the scene loop. Fixes: `engine3x.resolveOutputPath`/`unpackAsset` now run paths through new `stripDbPrefix` helper (regex `/^db:\/+(?:assets\/|internal\/)?/i`); the scene loop's `info.path` is honoured when `cfg.paths[uuid]` is absent; `recoverScripts` filters runtime files via module-level `RUNTIME_PATTERNS` + `isRuntimeScript` (narrow patterns: known exact names + engine-prefix + base62 hash, avoiding user-script false positives like `Player-Behavior.js`) and routes them to `<out>/_runtime/`; `projectScaffold.writeCocos3xProject` emits the **real** 3.8 schema verified against `cocos/cocos-test-projects v3.8.7`: `package.json` carries `name`/`type:'3d'`/`uuid` (uuidUtils-generated)/`version`/`creator.{version,dependencies}`; `settings/v2/packages/project.json` carries `general.designResolution`/`sortingLayers`/`layers`/`customJointTextureLayouts`. Legacy `project.json` and `settings/project.json` kept as 2.x-compat shims (3.8 ignores them). Tests in `test/unit/3x-cocos38-loadable.test.js` (12 cases including runtime-classifier coverage). `npm test`: 139 → **150 passing**, no regressions. Verified on `~/mini/slgq-reverse`: 0 `db:/` literal dirs (was 4), `assets/main/scene/game.fire` produced (was missing), 6 runtime files relocated to `_runtime/`, `package.json` carries Dashboard-required fields.
